			...
			for(Map.Entry<String, String> entry : mapResult.entrySet()){
				...
				if(!scanedQtyStr.isEmpty()){
		        	...
		        	if(orderPos == null){
		        		...
		        		orderPos.setFactQuantity((orderPos.getFactQuantity()==null?0:orderPos.getFactQuantity())+Long.decode(scanedQtyStr));
		        		if(orderPos.getExpectQuantity().longValue()==orderPos.getFactQuantity().longValue())
		        			orderPos.setStatusId(DefaultValue.SHIPPED_UNIT_STATUS);
					...
					new HistoryController().addEntryByShpt(sessinH, orderPos, unit, comment, Long.decode(session.getAttribute("userid").toString()));
			}

			if(new CheckManager().isFullOrderShiped(orderPos))
			{
				WmOrder order = new OrderManager().getOrderById(orderPos.getOrderId());
				new CheckController().setDocsStatus(sessinH, order, DefaultValue.SHIPPED_UNIT_STATUS);
			}
