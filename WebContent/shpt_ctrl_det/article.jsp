...
<p>
<big>Детал. Отгрузка</big> <small>${articlecode}</small><br>
<small>${articlename}</small><br>
<%=session.getAttribute("skuname")==null?"":session.getAttribute("skuname")%> <%=session.getAttribute("sku_delim")%>
<b>
<%=session.getAttribute("quantity")==null?"":session.getAttribute("quantity") %>
</b>
<br>
